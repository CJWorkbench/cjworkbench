...
class ColumnTypeNumberTests(unittest.TestCase):
    ...
    def test_custom_format(self):
        ...
        assert_series_equal(result,
                            pd.Series(['$1.10', '$2,231.00', np.nan, '$0.12']))
    ...
    def test_format_percent(self):
        ...
        result = column_type.format_series(series)
        assert_series_equal(result,
                            pd.Series(['30.0%', '1,111.1%', '0.0%', np.nan]))
...
class ColumnTypeDatetimeTests(unittest.TestCase):
    def test_format(self):
        series = pd.Series([dt(1999, 2, 3, 4, 5, 6, 7), np.nan,
                            dt(2000, 3, 4, 5, 6, 7, 8)])
        column_type = ColumnType.DATETIME()
        result = column_type.format_series(series)
        assert_series_equal(
            result,
            pd.Series(['1999-02-03T04:05:06.000007Z', np.nan,

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for provider in providers:
            ...
            if not created:
                self.stdout.write(self.style.SUCCESS('Found existing "%s" provider config' % provider['provider']))
                for field in provider_fields:
                    if provider[field] != getattr(social_app, field):
                        self.stdout.write(self.style.SUCCESS('Updating "%s" field' % field))
                        setattr(social_app, field, provider[field])
                        dirty = True
            else:
                self.stdout.write(self.style.SUCCESS('Created new "%s" provider config' % provider['provider']))

            if dirty:
                self.stdout.write(self.style.SUCCESS('Saving "%s" provider config' % provider['provider']))
                social_app.save()
            else:
                self.stdout.write(self.style.SUCCESS('No changes to "%s" provider config' % provider['provider']))

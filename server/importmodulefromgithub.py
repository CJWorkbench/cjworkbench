...
def import_module_from_directory(version: str, importdir: Path, force_reload=False):
    ...
    if not force_reload:
        ...
        try:
            ModuleVersion.objects.get(id_name=spec.id_name, source_version_hash=version)
            raise ValueError(
    ...
    minio.fput_directory_contents(minio.ExternalModulesBucket, prefix, Path(importdir))

...
def _get_lesson_or_404(stub):
    try:
        return Lesson.objects.get(stub)
...
def _render_get_lesson_detail(request, lesson):
    workflow = get_object_or_404(Workflow, owner=request.user, lesson_stub=lesson.stub)
...
@login_required
def render_lesson_detail(request, stub):
    lesson = _get_lesson_or_404(stub)

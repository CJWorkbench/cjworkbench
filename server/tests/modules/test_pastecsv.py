...
class PasteCSVTests(unittest.TestCase):
    ...
    def test_csv(self):
        ...
        expected = pd.DataFrame(
            {"A": [1, 2], "B": pd.Series(["foo", "bar"], dtype="category")}
        )
    ...
    def test_extra_data_should_not_mangle_index(self):
        ...
        assert_frame_equal(
            result, pd.DataFrame({"A": ["a"], "B": ["b"]}, dtype="category")
        )
    ...
    def test_no_header(self):
        ...
        assert_frame_equal(
            result,
            pd.DataFrame({"Column 1": ["A"], "Column 2": ["B"]}, dtype="category"),
        )
    ...
    def test_no_nan(self):
        ...
        result = render(P(csv="A,B\nx,y\nz,NA"))
        expected = pd.DataFrame({"A": ["x", "z"], "B": ["y", "NA"]}, dtype="category")
        assert_frame_equal(result, expected)

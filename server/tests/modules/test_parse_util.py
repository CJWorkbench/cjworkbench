...
class ParseJsonTests(SimpleTestCase):
    ...
    def test_json_mixed_types_are_str(self):
        ...
        expected = pd.DataFrame({'A': ['1', '2']})
        self.assertEqual(result['error'],
                         'Column "A" was mixed-type; we converted it to text.')
    ...
    def test_json_encode_nested_arrays_and_objects(self):
        ...
        assert_frame_equal(result, pd.DataFrame({'value': [
    ...
    def test_parse_too_many_rows(self):
        ...
        self.assertEqual(result['error'],
                         'The input had too many rows, so we removed rows.')
        assert_frame_equal(result['dataframe'],
                           pd.DataFrame({'A': ['a', 'b']}))
    ...
    def test_parse_too_many_columns(self):
        ...
        self.assertEqual(result['error'], (
            'The input had too many columns, so we removed some.'
        ))
        assert_frame_equal(result['dataframe'],
                           pd.DataFrame({'A': ['a', 'aa'], 'B': ['b', 'bb']}))

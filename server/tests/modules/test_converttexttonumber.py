...
from server.modules.converttexttonumber import render, InputNumberType, \
        InputLocale, Form, migrate_params
...
class TestExtractNumbers(unittest.TestCase):
    ...
    def test_extract_any_many_commas(self):
        result = Form(['A'], True, InputNumberType.ANY,
                      InputLocale.US).convert_table(pd.DataFrame({
            'A': ['1,234,567,890']
        }))
    ...
    def test_extract_any_us_thousands_must_be_in_groups_of_3(self):
        ...
        result = form.convert_table(pd.DataFrame({
    ...
    def test_match_eu_thousands_must_be_in_groups_of_3(self):
        form = Form(['A'], False, InputNumberType.ANY, InputLocale.EU,
                    error_means_null=True)
        result = form.convert_table(pd.DataFrame({
            'A': ['123.4', '2.345.1', '3.23,123']
        }))
    ...
    def test_extract_float_us(self):
        table = pd.DataFrame({'A': ['1,234', '2,345.67', '3.456']})
        form = Form(['A'], True, InputNumberType.FLOAT, InputLocale.US,
                    error_means_null=True)
        result = form.convert_table(table)
        assert_frame_equal(result,
    ...
    def test_extract_float_eu(self):
        table = pd.DataFrame({'A': ['1,234', '2,345.67', '3.456']})
        form = Form(['A'], True, InputNumberType.FLOAT, InputLocale.EU,
                    error_means_null=True)
        result = form.convert_table(table)

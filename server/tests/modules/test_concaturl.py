...
table = pd.DataFrame({"col1": ["a", "a"], "key": ["b", "c"]})

ext_workflow = pd.DataFrame({"key": ["b", "d"], "col2": ["c", "a"]})
...
class ConcatURLTest(unittest.TestCase):
    ...
    def test_concat_only_left_columns(self):
        ...
        assert_frame_equal(
            result,
            pd.DataFrame(
                {"col1": ["a", "a", np.NaN, np.NaN], "key": ["b", "c", "b", "d"]}
            ),
        )
    ...
    def test_concat_intersect_columns(self):
        ...
        assert_frame_equal(result, pd.DataFrame({"key": ["b", "c", "b", "d"]}))

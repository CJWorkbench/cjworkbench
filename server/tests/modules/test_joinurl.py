...
ref_left_join = pd.DataFrame(
    [["a", "b", "c", "d"]], columns=["col1", "key", "col2", "col3"]
)

table_with_types = pd.DataFrame([[1, 2], [1, 3]], columns=["col1", "key"])
ext_workflow_with_types = pd.DataFrame(
    [[2.0, 3.0, 4.0], [4.0, 1.0, 2.0]], columns=["key", "col2", "col3"]
)
...
class JoinURLTests(unittest.TestCase):
    ...
    def test_join(self):
        ...
        result = render(
            ...
            PR("", {"key": ["b", "d"], "col2": ["c", "a"], "col3": ["d", "b"]}),
        )
        assert_frame_equal(
            result,
            pd.DataFrame({"col1": ["a"], "key": ["b"], "col2": ["c"], "col3": ["d"]}),
        )

    def test_importcols(self):
        result = render(
            pd.DataFrame({"col1": ["a", "a"], "key": ["b", "c"]}),
            P(type="inner", colnames=["key"], select_columns=True, importcols="col2"),
            PR("", {"key": ["b", "d"], "col2": ["c", "a"], "col3": ["d", "b"]}),
        )
        self.assertEqual(["col1", "key", "col2"], list(result.columns))

    def test_colnames_not_in_right(self):
        result = render(
            pd.DataFrame({"A": [1], "B": [2]}),
            P(colnames=["A"], select_columns=True, importcols="B"),
            PR("", {"A": [1], "C": [2]}),
        )
    ...
    def test_fetch(self, inner_fetch):
        ...
        result = async_to_sync(fetch)(
            params, workflow_id=1, get_workflow_owner=get_workflow_owner
        )
    ...
    def test_fetch_invalid_url(self):
        ...
        self.assertEqual(
            result, ProcessResult(error="Not a valid Workbench workflow URL")
        )

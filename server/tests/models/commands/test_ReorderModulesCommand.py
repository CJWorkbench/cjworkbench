...
from server.models.commands import InitWorkflowCommand, ReorderModulesCommand
...
class ReorderModulesCommandTest(DbTestCase):
    ...
    def test_reorder_modules(self):
        ...
        wfm3 = self.tab.wf_modules.create(last_relevant_delta_id=v1, order=2)
    ...
    def test_missing_wf_module_valueerror(self):
        wfm1 = self.tab.wf_modules.create(last_relevant_delta_id=self.delta.id, order=0)
        with self.assertRaises(ValueError):
            self.run_with_async_db(
                ReorderModulesCommand.create(
    ...
    def test_not_enough_wfmodules_valueerror(self):
        wfm1 = self.tab.wf_modules.create(order=0, last_relevant_delta_id=self.delta.id)
        self.tab.wf_modules.create(order=1, last_relevant_delta_id=self.delta.id)
    ...
    def test_no_change_does_nothing(self):
        wfm1 = self.tab.wf_modules.create(order=0, last_relevant_delta_id=self.delta.id)
        wfm2 = self.tab.wf_modules.create(order=1, last_relevant_delta_id=self.delta.id)

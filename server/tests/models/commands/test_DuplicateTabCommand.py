...
class DuplicateTabCommandTest(DbTestCase):
    ...
    def test_duplicate_empty_tab(self, ws_notify, queue_render):
        ...
        cmd = self.run_with_async_db(DuplicateTabCommand.create(
            workflow=workflow,
            from_tab=tab,
            slug='tab-2',
            name='Tab 2',
    ...
    def test_duplicate_nonempty_unrendered_tab(self, ws_notify, queue_render):
        ...
        self.assertEqual(wfm1dup.last_relevant_delta_id,
        ...
        self.assertEqual(delta['updateTabs']['tab-2']['wf_module_ids'],
                         [wfm1dup.id, wfm2dup.id])
        self.assertEqual(list(delta['updateWfModules'].keys()),
                         # dict preserves order
                         [str(wfm1dup.id), str(wfm2dup.id)])
        wfm1update = delta['updateWfModules'][str(wfm1dup.id)]
        self.assertEqual(wfm1update['last_relevant_delta_id'],
                         wfm1.last_relevant_delta_id)
        ...
        self.assertEqual(delta['clearWfModuleIds'],
                         # dict preserves order
                         [str(wfm1dup.id), str(wfm2dup.id)])

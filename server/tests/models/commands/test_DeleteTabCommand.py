...
from server.models.commands import DeleteTabCommand
...
class DeleteTabCommandTest(DbTestCase):
    ...
    def test_delete_tab(self):
        ...
        cmd = self.run_with_async_db(DeleteTabCommand.create(workflow=workflow,
                                                             tab=tab2))
    ...
    def test_ws_data(self, send_delta):
        ...
        self.assertEqual(delta1['updateWorkflow']['tab_ids'], [tab1.id])
        self.assertFalse('updateTabs' in delta1)
        self.assertEqual(delta1['clearTabIds'], [tab2.id])

        self.run_with_async_db(cmd.backward())
        delta2 = send_delta.call_args[0][1]
        self.assertEqual(delta2['updateWorkflow']['tab_ids'],
                         [tab1.id, tab2.id])
        self.assertEqual(list(delta2['updateTabs'].keys()), [str(tab2.id)])
        self.assertFalse('clearTabIds' in delta2)

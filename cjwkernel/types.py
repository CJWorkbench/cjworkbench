...
class UploadedFile(NamedTuple):
    ...
    This value is generated by Workbench. It is "safe" to open.
    """
...
def arrow_uploaded_file_to_thrift(value: UploadedFile) -> ttypes.UploadedFile:
    return ttypes.UploadedFile(
        name=value.name,
        filename=value.filename,
        uploaded_at_timestampus=int(value.uploaded_at.timestamp() * 1000000),
    )
...
def thrift_uploaded_file_to_arrow(value: ttypes.UploadedFile) -> UploadedFile:
    return UploadedFile(
        name=value.name,
        filename=value.filename,
        uploaded_at=datetime.datetime.utcfromtimestamp(
            value.uploaded_at_timestampus / 1000000.0
        ),
    )

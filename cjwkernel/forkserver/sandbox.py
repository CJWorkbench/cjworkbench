...
def sandbox_child_from_forkserver(child_pid: int, config: SandboxConfig) -> None:
    """
    Sandbox the child process from the forkserver side of things.
    ...
    if config.network is not None:
        _setup_network_namespace_from_forkserver(config.network, child_pid)
...
def sandbox_child_self(config: SandboxConfig) -> None:
    ...
    This must not be called before forkserver finishes calling
    sandbox_child_from_forkserver().
    """
    _Sandbox(config).run()

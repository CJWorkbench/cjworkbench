...
describe('ReorderColumns actions', () => {
  it('adds a new reorder module', () => {
    ...
    expect(ret).toEqual({
      'reorder-history': JSON.stringify([{ column: 'A', to: 0, from: 3 }])
    })
  })

  it('updates a reorder module', () => {
    const ret = func(
      { 'reorder-history': JSON.stringify([{ column: 'A', from: 2, to: 0 }]) },
      { column: 'B', from: 1, to: 2 }
    )
    expect(ret).toEqual({
      'reorder-history': JSON.stringify([
        { column: 'A', from: 2, to: 0 },
        { column: 'B', to: 1, from: 1 }
      ])
    })
  })
})

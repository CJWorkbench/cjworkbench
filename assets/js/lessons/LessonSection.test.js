...
describe('LessonSection', () => {
  ...
  it('renders .not-current when !isCurrent', () => {
    expect(wrapper({ isCurrent: false }).prop('className')).toMatch(
      /\bnot-current\b/
    )
  })
  ...
  it('renders the description HTML', () => {
    expect(
      wrapper()
        .find('.description')
        .html()
    ).toEqual('<div class="description"><p>Section One HTML</p></div>')
  })
  ...
  it('renders steps as FUTURE when index<activeStepIndex', () => {
    ...
    expect(w.find('LessonStep').map(s => s.prop('status'))).toEqual([
      'future',
      'future',
      'future'
    ])

...
describe('Secret/String', () => {
  ...
  describe('without a secret', () => {
    ...
    it('renders a text box', () => {
      ...
      expect(w.find('input[data-name="x"]').prop('pattern')).toEqual(
        '\\w\\w\\w\\d\\d\\d'
      )
    ...
    it('prevents submitting input not matching pattern', () => {
      w.find('input[data-name="x"]').simulate('change', {
        target: { value: 'abc', validity: { valid: false } }
      })
    ...
    it('submits, disabling input and button immediately', () => {
      w.find('input[data-name="x"]').simulate('change', {
        target: { value: 'abc123', validity: { valid: true } }
      })
  ...
  describe('with a secret', () => {
    let w
    beforeEach(() => {
      w = wrapper({ secretMetadata: { name: '2019-06-10T15:40:12.000Z' } })
    })
    afterEach(() => {
      w.unmount()
      w = null
    ...
    it('renders a timestamp', () => {
      ...
      expect(w.find('time').prop('dateTime')).toEqual(
        '2019-06-10T15:40:12.000Z'
      )

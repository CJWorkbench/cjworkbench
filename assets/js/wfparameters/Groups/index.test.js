...
describe('Groups', () => {
  const wrapper = (extraProps={}) => mount(
    <Groups
      ...
      allColumns={[{name: 'A', type: 'text'}, {name: 'B', type: 'datetime'}]}
      onChange={jest.fn()}
  ...
  it('should show dropdown only for selected dates', () => {
    const w = wrapper({
      ...
      allColumns: [{name: 'A', type: 'datetime'}, {name: 'B', type: 'datetime'}]
    })
  ...
  it('should set date granularity when there is no value', () => {
    const w = wrapper({
      name: 'w',
      value: {colnames: 'A', group_dates: true, date_granularities: {}},
      allColumns: [{name: 'A', type: 'datetime'}, {name: 'B', type: 'text'}]
    })

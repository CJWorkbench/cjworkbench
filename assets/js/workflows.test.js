...
describe('Workflow list page', () => {
  ...
  it('delete a workflow', (done) => {
    ...
    setImmediate(() => {
      ...
      setImmediate(() => {
        ...
        expect(w.find('.tab-pane.active').find('.workflow-item')).toHaveLength(2) // one fewer workflow
        done()
      })
    })
  })
  ...
  it('duplicate workflow callback', (done) => {
    ...
    setImmediate( () => {
      ...
      setImmediate(() => {
        ...
        expect(w.find('.tab-pane.active').find('.workflow-item')).toHaveLength(4)
        done()
      })
    })
  })
  ...
  it('owned pane should have create workflow link when no workflows', (done) => {
    ...
    setImmediate( () => {
      ...
      expect(w.find('.tab-pane.active').find('.new-workflow-link')).toHaveLength(1)
      done()
    })
  })
  ...
  it('should sort properly by date and name', (done) => {
    ...
    setImmediate( () => {
      ...
      w.update()
      w.find('button.test-sort-date-ascending').simulate('click')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(0).key).toEqual('2')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(1).key).toEqual('1')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(2).key).toEqual('3')
      // sort by date descending
      w.update()
      w.find('button.test-sort-date-descending').simulate('click')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(0).key).toEqual('3')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(1).key).toEqual('1')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(2).key).toEqual('2')
      // sort by name ascending
      w.update()
      w.find('button.test-sort-name-ascending').simulate('click')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(0).key).toEqual('3')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(1).key).toEqual('2')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(2).key).toEqual('1')
      // sort by name descending
      w.update()
      w.find('button.test-sort-name-descending').simulate('click')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(0).key).toEqual('1')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(1).key).toEqual('2')
      expect(w.find('.tab-pane.active').find('.workflow-item').get(2).key).toEqual('3')
      done()
    })

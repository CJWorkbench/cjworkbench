...
describe('Workflow list page', () => {
  ...
  it('renders My workflows correctly', () => {
    const { container } = renderUi({ currentPath: '/workflows', workflows: testOwnedWorkflows })
  ...
  it('renders Shared correctly', () => {
    const { container } = renderUi({ currentPath: '/workflows/shared-with-me', workflows: testSharedWorkflows })
    expect(container).toMatchSnapshot()
  ...
  it('renders Examples correctly', () => {
    const { container } = renderUi({ currentPath: '/workflows/examples', workflows: testExampleWorkflows })
    expect(container).toMatchSnapshot()
  ...
  it('deletes a workflow', async () => {
    ...
    const { container, getByText, queryAllByTitle } = renderUi({ currentPath: '/workflows', workflows: testOwnedWorkflows })
    ...
    act(() => { fireEvent.click(getByText('Delete')) })
  ...
  it('duplicates a workflow', async () => {
    ...
    fireEvent.click(queryAllByTitle('menu')[1]) // [0] opens the nav.main-nav menu
    act(() => { fireEvent.click(getByText('Duplicate')) })

    expect(api.duplicateWorkflow).toHaveBeenCalledWith(3) // 3 shows up on top
    expect(container.querySelectorAll('tbody>tr')).toHaveLength(3)
    expect(container.querySelector('tr.changing')).not.toBe(null)

    resolvePromise({ id: 12, name: 'Copied!', owner_name: 'Fred Frederson', public: false, is_owner: true, last_update: '2021-02-08T15:56.000Z' })
  ...
  it('should have a placeholder in the Shared pane', () => {
    const { getByText } = renderUi({ currentPath: '/workflows/shared-with-me', workflows: [] })
    getByText(/Workflows shared with you as collaborator will appear here/)
  })

  it('should have a placeholder in the Examples pane', () => {
    const { getByText } = renderUi({ currentPath: '/workflows/examples', workflows: [] })
    getByText('Publish workflows as examples using Django admin')
  })

  it('should sort by updated_at', async () => {
    const { container, getByText } = renderUi({ currentPath: '/workflows/examples', workflows: testOwnedWorkflows })

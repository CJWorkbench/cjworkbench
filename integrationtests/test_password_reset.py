...
class TestPasswordReset(WorkbenchBase):
    ...
    def test_password_reset_wrong_email(self):
        ...
        self.browser.assert_element('ul.errorlist>li',
                                    text='not assigned to any user account')
    ...
    def test_try_reset_to_invalid_passwords(self):
        ...
        b.assert_element('ul.errorlist',
                         text='You must type the same password each time')

        self._try_reset_password('user@example.org', 'user@example.org')
        b.assert_element('ul.errorlist',
                         text='too similar to the email address')
